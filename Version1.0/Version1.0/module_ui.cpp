...
void buzzer_init() {
    // ESP32 Arduino Core 3.x LEDC 设置：通道、频率 (Hz)、分辨率 (位)
    ledcAttach(BUZZER_PIN, 2000, 8);
    ledcWriteTone(BUZZER_PIN, 0);  // 静音
}
...
static void startMelody(const Note* notes, uint8_t len) {
    ...
    if (melodyBuf[0].freq > 0) {
        ledcWriteTone(BUZZER_PIN, melodyBuf[0].freq);
    } else {
        ledcWriteTone(BUZZER_PIN, 0);
    }
    ...
    if (melodyPlaying) {
        if (now - noteStartTime >= melodyBuf[melodyIdx].durationMs) {
            ...
            if (melodyBuf[melodyIdx].freq > 0) {
                ledcWriteTone(BUZZER_PIN, melodyBuf[melodyIdx].freq);
            } else {
                ledcWriteTone(BUZZER_PIN, 0);
            }
            noteStartTime = now;
